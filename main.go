...
// isGoogleAPIType returns true for googleapis/type/* messages (e.g. PostalAddress)
// that are external genproto types without option helpers generated by this plugin.
// Date is excluded because it has its own special handling in generateNestedFieldOption.
func isGoogleAPIType(ident protogen.GoIdent) bool {
	return strings.Contains(ident.GoImportPath.String(), "googleapis/type/") && ident.GoName != "Date"
}
...
func generateNestedFieldOption(g *protogen.GeneratedFile, message *protogen.Message, field *protogen.Field, optionName string) {
	// googleapis types (e.g. PostalAddress) are external genproto types that don't
	// have option helpers generated by this plugin. Skip WithNew* entirely; the
	// direct setter from generateDirectNestedFieldOption is sufficient.
	if isGoogleAPIType(field.Message.GoIdent) {
		log(g, "skipping nested option for googleapis type: ", field.Message.GoIdent.GoName)
		return
	}
